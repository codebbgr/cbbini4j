 ...
 */
/**
 * Changelog ========= 03/04/2020 (gmoralis) - Refactored some stuff with jdk5+ style 03/04/2020
 * (gmoralis) - Added Charset in FileReader,FileWriter (JDK11+) 03/04/2020 (gmoralis) - Initial
...
public class ConfigParser implements Serializable {
  ...
  public void read(File file) throws IOException, ParsingException {
    try {
      _ini.load(new FileReader(file, StandardCharsets.UTF_8));
    } catch (InvalidFileFormatException x) {
      throw new ParsingException(x);
    }
  }
  ...
  public void write(File file) throws IOException {
    _ini.store(new FileWriter(file, StandardCharsets.UTF_8));
  }
